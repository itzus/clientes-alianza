...
export class ClienteListComponent implements OnInit {
  ...
  loadClientes(): void {
    ...
    this.isLoading = true;
    this.clienteService.getClientes(this.pageIndex, this.pageSize).subscribe({
  ...
  buscarPorSharedKey(): void {
    ...
    this.isLoading = true;
    this.clienteService.getClienteBySharedKey(this.sharedKeySearch).subscribe({
      next: (cliente) => {
      ...
      error: (error) => {
        if (error.status === 404) {
          ...
          this.snackBar.open(
            "No se encontró ningún cliente con esa clave",
            "Cerrar",
            { duration: 3000 }
          );
        } else {
  ...
  aplicarFiltros(): void {
    ...
    this.isLoading = true;
    this.clienteService
      .searchClientes(this.filtro, this.pageIndex, this.pageSize)
      .subscribe({
        next: (response: PageResponse<Cliente>) => {
          console.log(response);
          this.clientes = response.content;
        ...
        error: (error) => {
          ...
          this.snackBar.open("Error al filtrar clientes", "Cerrar", {
            duration: 3000,
          });
          this.isLoading = false;
        },
  ...
  exportarCSV(): void {
    this.isLoading = true;
    this.clienteService.exportClientesToCsv().subscribe({
      next: (blob: Blob) => {
        ...
        this.isLoading = false;
        this.snackBar.open("Archivo CSV descargado correctamente", "Cerrar", {
          duration: 3000,
        });
      },
      error: (error) => {
        console.error("Error al exportar clientes", error);
        this.snackBar.open("Error al exportar clientes a CSV", "Cerrar", {
          duration: 3000,
        });
        this.isLoading = false;
      },
  ...
  nuevoCliente(): void {
    this.abrirModalCliente(undefined);
  }
